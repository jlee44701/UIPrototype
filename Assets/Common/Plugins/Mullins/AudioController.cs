...
public class AudioController : MonoBehaviour {
    ...
    private readonly int[] DEFAULT_LOOPSOURCE_INDICES = new int[] {
        0
    };
    ...
    void Awake() {
        ...
        AssetReference asd = new AssetReference();
        var thing = AddressablesUtility.GetAddressFromAssetReference(asd);
        Instance = this;
    ...
    private IEnumerator CrossFade(string newLoop, float volume, float duration, float newLoopStartTimeNormalized, int sourceIndex = 0) {
        if (loopSources[0].clip && loopSources[0].isPlaying) {
